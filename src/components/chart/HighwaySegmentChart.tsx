  ...
  years.forEach((yr) => {
    ...
    const yearSegments = data
      ...
      .map((f) => {
        ...
        const endMarker = cleanAndRound(f.properties.TX_END_REF_MARKER_NBR)
        const endDisp = cleanAndRound(f.properties.TX_END_REF_MARKER_DISP)

        // Calculate begin and end using the rounded values
        const begin = beginMarker + beginDisp
        const end = endMarker + endDisp
        const score = Number(f.properties[selectedScore.value])

        // Ensure minimum segment length for visibility
        const segmentLength = end - begin
