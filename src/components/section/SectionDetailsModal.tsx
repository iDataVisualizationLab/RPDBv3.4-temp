    ...
    return (
        ...
        > */}
            <Rnd
                ...
                onResizeStop={() => { isResizing.current = false; }}
                minWidth={300}
                minHeight={300}
                maxWidth={1200}
                bounds="window"
                                ...
                                <div className="p-4 border border-gray-200 rounded-lg mt-2 bg-white shadow-sm">
                                    ...
                                    <div className="bg-white p-3 rounded-lg border shadow-sm mb-6">
                                        ...
                                        {surveyData.Deflection?.length > 0 ? (
                                            <TabbedChart
                                                years={surveyData.Deflection.map((item: { DMI: number }) => item.DMI)}
                                                dataSets={Object.keys(surveyData.Deflection[0] || {}).filter(k => k !== 'DMI').map(key => ({ label: key, fill: false, data: surveyData.Deflection.map((d: any) => d[key]) }))}
                                            />
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {error ? (
                                            ...
                                            Object.entries(groupedImages).map(([date, imgs]) => {
                                                const label = convertDate(date) ?? date;
                                                return (
