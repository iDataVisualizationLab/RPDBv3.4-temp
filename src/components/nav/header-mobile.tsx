...
const MenuItemWithSubMenu = ({
  ...
  return (
    <>
      <MenuItem>
        <button
          className="flex w-full text-2xl px-2 py-1 rounded-md transition-colors duration-150 hover:bg-[var(--theme-hover)]"
          ...
          item.subMenuItems?.map((subItem, subIdx) => (
            <MenuItem key={subIdx}>
              <Link
                href={subItem.path}
                onClick={() => toggleOpen()}
                className={`text-base ${subItem.path === pathname ? 'font-bold' : ''}`}
              >
                {subItem.title}
              </Link>
            </MenuItem>
