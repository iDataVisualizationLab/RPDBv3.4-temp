...
const HeatMapModal = dynamicImport(() => import("./HeatMapModal"), {
  ssr: false,
})
...
const EXP3: React.FC = () => {
  ...
  const [scoreGauges, setScoreGauges] = useState<{
    [k in "condition" | "distress" | "ride" | "aadt" | "cost"]?: number
  }>({})
  const [mapModalInfo, setMapModalInfo] = useState<{
    highway: string
    county: string
  } | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`${routePublic}/files/Concrete_distresses.csv`)
  ...
  useEffect(() => {
    if (activeHeatMapData.length === 0) {
      // Clear all gauge data when no heatmaps are active
      setScoreGauges({})
      setSelectedCharts([])
      // Reset the heatmap ID counter
      heatmapIdCounter.current = 0
      // Set recently cleared flag
      setRecentlyCleared(true)
    } else {
      // Reset recently cleared flag when we have active heatmaps
  ...
  useEffect(() => {
    // Create a new object to hold active score types
    ...
    // Scan all active heatmaps and their visible scores
    activeHeatMapData.forEach((heatmap) => {
      heatmap.scores.forEach((score) => {
        ...
        if (scoreType) {
          activeScoreTypes[scoreType] = true
        }
      })
    })

    // Update scoreGauges to only include active score types
    setScoreGauges((prev) => {
      const newGauges: typeof prev = {}

      // Only keep gauges for active score types
      Object.entries(prev).forEach(([type, value]) => {
        if (activeScoreTypes[type]) {
          newGauges[type as keyof typeof prev] = value
        }
      })
  ...
  const addChart = useCallback(
    (chart: ChartData, scoreValue: number) => {
      if (recentlyCleared) {
        ...
        if (scoreType) {
          setScoreGauges({ [scoreType]: scoreValue })
        }
      } else {
        ...
        const scoreType = fieldToScoreType[chart.field as keyof typeof fieldToScoreType]
        if (scoreType) {
          setScoreGauges((prev) => ({ ...prev, [scoreType]: scoreValue }))
        }
      }
    },
    [recentlyCleared],
  ...
  const addOrUpdateHeatMapData = useCallback(
    (chart: ChartData) => {
      // Use specific labels for known field types
      ...
      switch (chart.field) {
        ...
        case "TX_MAINTENANCE_COST_AMT":
          scoreLabel = "Maintenance Cost"
          break
        case "TX_CONDITION_SCORE":
          scoreLabel = "Condition Score"
          break
        case "TX_DISTRESS_SCORE":
          scoreLabel = "Distress Score"
          break
        case "TX_RIDE_SCORE":
          scoreLabel = "Ride Score"
          break
        default:
          scoreLabel = chart.field
            .replace("TX_", "")
            .split("_")
        ...
        setActiveHeatMapData((prev) => {
          ...
          if (!scoreExists) {
            updated[existingIndex] = {
              ...updated[existingIndex],
              scores: [...updated[existingIndex].scores, newScore],
            }
          }
          return updated
        })
  ...
  const showMapModal = useCallback((highway: string, county: string) => {
    setMapModalInfo({ highway, county })
  ...
  const closeMapModal = useCallback(() => {
    ...
    setMapModalInfo(null)
  }, [])

  // Add the following state for managing the draggable split after the existing state declarations:
  ...
  // Add this mouse event handler function after the existing useCallback functions:
  ...
  // Add a function to close all heatmaps after the existing callback functions:
  const closeAllHeatmaps = useCallback(() => {
    setActiveHeatMapData([])
  }, [])

  // Memoize visualization components to prevent unnecessary re-renders
  ...
  // Simplified table modal component with search props
  const tableModalComponent = useMemo(
    () => (
      <TableModalPMIS
        title="PMIS Data Lines"
  ...
  // Replace the entire return statement with this updated layout that includes the draggable divider and close all button:
  return (
    <div>
      <div ref={containerRef} className="flex h-screen w-full p-4 bg-gray-100 overflow-hidden gap-1">
        {/* Left side - Table */}
        <div
        ...
        >
          <div className="flex-grow overflow-auto">{tableModalComponent}</div>
        </div>
              ...
              {activeHeatMapData.map((data) => (
                <div key={data.id} className="mb-4">
                  <HeatMapModal
                    ...
                    features={pmisFeatures}
                    onClose={() => {
                      setActiveHeatMapData((prev) => prev.filter((item) => item.id !== data.id))
                    }}
                        ...
                        return prev
                          .map((item) => {
                            if (item.id === data.id) {
                              return {
                                ...item,
                                scores: item.scores.filter((s) => s.value !== scoreValue),
                              }
                            }
                            return item
                          })
      ...
      {mapModalOpen && mapModalInfo && (
        <MapModal
          ...
          highway={mapModalInfo.highway}
          county={mapModalInfo.county}
        />
      )}
    </div>
  )
