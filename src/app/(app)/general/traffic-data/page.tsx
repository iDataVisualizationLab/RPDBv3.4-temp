  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (highlightLayerRef.current) {
        const intervalId = (highlightLayerRef.current as any)["pulseIntervalId"]
        ...
        view
          ...
          .then(async (response: any) => {
            ...
            if (aadtFeature) {
              ...
              const baseHighlight = new Graphic({
                geometry: aadtFeature.geometry,
                symbol: {
                  type: "simple-marker",
                  color: [252, 141, 98, 0.8], // This matches the #fc8d62 color
                  size: 8,
                  outline: {
                    color: [202, 91, 48, 1], // Darker version for outline
                    width: 3,
                  },
                },
              })
              highlightLayer.add(baseHighlight)

              const outerPulse = new Graphic({
                geometry: aadtFeature.geometry,
                symbol: {
                  type: "simple-marker",
                  color: [0, 0, 0, 0],
              ...
              const pulseInterval = setInterval(() => {
                ...
                if (outerPulse.symbol && "outline" in outerPulse.symbol && outerPulse.symbol.outline?.color) {
                  const currentColor = [0, 0, 0, pulseState ? 0.7 : 0.2] // Black with varying opacity
                  outerPulse.symbol.outline.color = currentColor

                  if ("size" in outerPulse.symbol) {
                    outerPulse.symbol.size = pulseState ? 28 : 24
                  }
                }

                if (baseHighlight.symbol && "outline" in baseHighlight.symbol && baseHighlight.symbol.outline) {
                  baseHighlight.symbol.outline.width = pulseState ? 4 : 2
                }

                pulseState = !pulseState

                if (highlightLayerRef.current) {
                  highlightLayerRef.current.graphics.forEach((graphic: any) => {
                    if (graphic) {
                      graphic.visible = false
                      setTimeout(() => {
                        if (graphic) graphic.visible = true
                      }, 0)
                    }
                  })
                }
              ...
              const oldIntervalId = (highlightLayerRef.current as any)["pulseIntervalId"]
              if (oldIntervalId) clearInterval(oldIntervalId)
              ;(highlightLayerRef.current as any)["pulseIntervalId"] = pulseInterval
